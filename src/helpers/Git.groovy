...
def tag(String version) {
    sh "git remote update; git fetch --all; git checkout main; git pull origin main; git tag ${version}; git push origin main --tags"
}
...
def release(String version) {
    ...
    withCredentials([gitUsernamePassword(credentialsId: 'girhub-credentials-pass',
                 gitToolName: 'default')]){
                         sh "git reset --hard HEAD; git checkout -b release-${version};git tag -a ${version}; git push origin release-${version} --tags"
